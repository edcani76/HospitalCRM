...
// Google Drive helpers for sample lab report uploads
async function getDriveAccessToken() {
  ...
  if (!resp.ok) {
    console.log('   [GDrive] Token refresh failed, skipping uploads');
    return null;
  }
...
async function uploadSampleLabReport(token: string, orderData: any, patientData: any, ownerData: any): Promise<string> {
  try {
    // Create folder structure: <ownerName>/<petName>/lab-reports
    let parentId = process.env.VITE_GOOGLE_DRIVE_FOLDER_ID || null;
    const ownerName = ownerData.displayName || ownerData.name || 'Owner';
    const petName = patientData.name || 'Patient';
    
    parentId = await getOrCreateFolder(token, ownerName, parentId);
    parentId = await getOrCreateFolder(token, petName, parentId);
    parentId = await getOrCreateFolder(token, 'lab-reports', parentId);

    const safeName = `${petName}_${(orderData.testName || 'Test').replace(/[^a-zA-Z0-9]/g, '_')}.pdf`.slice(0, 100);

    // Generate PDF Buffer using the React component
    ...
    const metadataResp = await fetch('https://www.googleapis.com/drive/v3/files?fields=id,name,webViewLink', {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ name: safeName, mimeType: 'application/pdf', parents: [parentId] }),
    });
    const fileData = await metadataResp.json();
    ...
    // Use thumbnail URL for rendering
    const thumbnailUrl = `https://drive.google.com/thumbnail?id=${fileData.id}&sz=w1000`;
    console.log(`   [GDrive] Uploaded lab report: ${safeName}`);
    return thumbnailUrl;
  } catch (err) {
    console.log(`   [GDrive] Upload failed for ${testName}: ${err}`);
    return '';
  }
}
...
async function seedEndToEndData() {
  ...
  await clearCollection('medications');
  ...
  const medicationsList = [
    { name: 'Amoxicillin-Clavulanate', category: 'Antibiotic', unit: 'tablets', price: 35, costPrice: 15, minStock: 50, reorderPoint: 100 },
    { name: 'Carprofen 75mg', category: 'Anti-inflammatory', unit: 'tablets', price: 45, costPrice: 20, minStock: 20, reorderPoint: 40 },
    { name: 'Metoclopramide 10mg', category: 'Other', unit: 'tablets', price: 15, costPrice: 5, minStock: 40, reorderPoint: 80 },
    { name: 'Metronidazole 250mg', category: 'Antibiotic', unit: 'tablets', price: 20, costPrice: 8, minStock: 30, reorderPoint: 60 },
    { name: 'Glucosamine Complex', category: 'Supplement', unit: 'chewable', price: 28, costPrice: 12, minStock: 20, reorderPoint: 40 },
    { name: 'Fluid Therapy (SubQ)', category: 'Supply', unit: 'bag', price: 200, costPrice: 80, minStock: 10, reorderPoint: 20 },
    { name: 'Meloxicam Oral Suspension', category: 'Anti-inflammatory', unit: 'ml', price: 85, costPrice: 30, minStock: 5, reorderPoint: 10 },
    { name: 'Feliway Diffuser', category: 'Supply', unit: 'unit', price: 950, costPrice: 450, minStock: 5, reorderPoint: 10 },
    { name: 'Doxycycline 100mg', category: 'Antibiotic', unit: 'tablets', price: 40, costPrice: 18, minStock: 30, reorderPoint: 60 },
    { name: 'Hydrocodone-Homatropine', category: 'Other', unit: 'ml', price: 55, costPrice: 20, minStock: 15, reorderPoint: 30 },
    { name: "Hill's Metabolic Diet", category: 'Diet', unit: 'bag', price: 1200, costPrice: 600, minStock: 10, reorderPoint: 20 },
    { name: 'Tramadol 50mg', category: 'Pain Relief', unit: 'tablets', price: 30, costPrice: 10, minStock: 40, reorderPoint: 80 },
    { name: 'Apoquel 16mg', category: 'Other', unit: 'tablets', price: 95, costPrice: 50, minStock: 30, reorderPoint: 60 },
    { name: 'Cephalexin 500mg', category: 'Antibiotic', unit: 'capsule', price: 25, costPrice: 10, minStock: 40, reorderPoint: 80 },
    { name: 'Chlorhexidine Shampoo', category: 'Supply', unit: 'bottle', price: 450, costPrice: 200, minStock: 10, reorderPoint: 20 },
    { name: 'Joint Supplement', category: 'Supplement', unit: 'bottle', price: 850, costPrice: 400, minStock: 15, reorderPoint: 30 }
  ];

  interface MedResult { id: string; name: string; price: number };
  const createdMeds: MedResult[] = [];
  console.log('📦 Creating E2E medications...');
  for (const med of medicationsList) {
    ...
    createdMeds.push({ id: ref.id, name: med.name, price: med.price });
  }

  console.log('📋 Creating inventory batches...');
  const batches: Array<{ id: string; medicationId: string; medicationName: string; batchNo: string; quantity: number }> = [];
  for (const med of createdMeds) {
    const qty = Math.floor(Math.random() * 80) + 120; // Ensure plenty of stock for dispensing
    const batchNo = `BATCH-${med.name.substring(0, 4).toUpperCase()}-001`;
    const ref = await addDoc(collection(db, 'inventory_batches'), {
      ...
      costPrice: medicationsList.find(m => m.name === med.name)?.costPrice || 0,
    ...
    batches.push({ id: ref.id, medicationId: med.id, medicationName: med.name, batchNo, quantity: qty });
    ...
    await addDoc(collection(db, 'stock_movements'), {
      ...
      quantity: qty,
      runningBalance: qty,
      reference: `REC-${batchNo}`,
      notes: 'Initial E2E stock receiving',
      userId: 'system',
      userName: 'System Admin',
      createdAt: serverTimestamp(),
    });

    await updateDoc(doc(db, 'medications', med.id), { stock: qty, updatedAt: serverTimestamp() });
  }
  console.log(`   ${batches.length} batches created and stock updated\n`);
  // --- END PHARMACY SETUP ---
  ...
  const visitScenarios: Array<{
    ...
    visits: Array<{
      ...
      labs?: Array<{ name: string; code: string; status: string; result?: string; priority?: string; testCategory?: string; sampleType?: string; billingStatus?: string; reason?: string; resultFileUrls?: string[] }>;
      ...
      visits: [
        {
          ...
          labs: [
            { name: 'Complete Blood Count', code: 'CBC001', status: 'completed', result: 'WBC 8.5 (ref 6-17), RBC 6.2 (ref 5.5-8.5), HGB 14.1 (ref 12-18), HCT 42% (ref 37-55), PLT 250 (ref 200-500) — All values within normal range', priority: 'Routine', testCategory: 'Laboratory', sampleType: 'Blood', billingStatus: 'paid', reason: 'Annual wellness screening' },
            { name: 'Serum Chemistry', code: 'CHE001', status: 'completed', result: 'ALT 142 U/L (ref 10-100) — mildly elevated, monitor. BUN 18 (ref 7-27), CREA 0.9 (ref 0.5-1.5), GLU 95 (ref 70-120) — otherwise normal', priority: 'Routine', testCategory: 'Laboratory', sampleType: 'Blood', billingStatus: 'paid', reason: 'Annual wellness screening' }
          ],
        ...
        },
        {
          ...
          labs: [
            { name: 'Fecal Floatation', code: 'FEC001', status: 'completed', result: 'No ova, cysts, or parasites detected on microscopic examination. Negative.', priority: 'Routine', testCategory: 'Laboratory', sampleType: 'Feces', billingStatus: 'paid', reason: 'GI workup for vomiting/diarrhea' }
          ],
          prescriptions: [
        ...
        },
        {
          ...
          labs: [
            { name: 'Complete Blood Count', code: 'CBC002', status: 'completed', result: 'WBC 9.2 (ref 6-17), RBC 6.8 (ref 5.5-8.5), HGB 15.0 (ref 12-18) — all within normal range', priority: 'Routine', testCategory: 'Laboratory', sampleType: 'Blood', billingStatus: 'paid', reason: 'Pre-op screening' },
            { name: 'Hip Dysplasia Panel', code: 'HIP001', status: 'completed', result: 'Norberg angle 100° bilaterally. Mild bilateral hip dysplasia, OFA Grade 1. Recommend weight management and joint supplements.', priority: 'Routine', testCategory: 'Imaging', sampleType: 'Radiograph', billingStatus: 'paid', reason: 'Limping hind legs' }
          ],
          prescriptions: [
      ...
      visits: [
        {
          ...
          labs: [
            { name: 'Urinalysis', code: 'URIN001', status: 'completed', result: 'Color: Yellow, Turbidity: Cloudy, pH 8.0, Specific Gravity 1.025, Protein 2+, WBC >50/hpf, RBC 5-10/hpf, Bacteria: Many rods. Impression: Active UTI with pyuria.', priority: 'Urgent', testCategory: 'Laboratory', sampleType: 'Urine', billingStatus: 'paid', reason: 'Lethargy, suspected UTI' },
            { name: 'CBC + Chemistry', code: 'CBC003', status: 'completed', result: 'BUN 35 (ref 7-27), CREA 1.8 (ref 0.5-1.5) — mildly elevated consistent with dehydration. WBC 18.5 (ref 6-17) — mild leukocytosis.', priority: 'Urgent', testCategory: 'Laboratory', sampleType: 'Blood', billingStatus: 'paid', reason: 'Lethargy workup' }
          ],
          prescriptions: [
        ...
        },
        {
          ...
          labs: [
            { name: 'Pre-Anesthetic Blood Panel', code: 'PAB001', status: 'completed', result: 'ALT 45 (ref 10-100), BUN 16 (ref 7-27), CREA 0.8 (ref 0.5-1.5), PT/PTT normal. All values within acceptable range for anesthesia.', priority: 'Routine', testCategory: 'Laboratory', sampleType: 'Blood', billingStatus: 'paid', reason: 'Pre-dental screening' },
            { name: 'Dental Panel', code: 'DNT001', status: 'completed', result: 'Grade 3 periodontal disease confirmed on dental exam under anesthesia. Severe gingivitis and bone loss noted on molars.', priority: 'Routine', testCategory: 'Imaging', sampleType: 'Dental X-Ray', billingStatus: 'paid', reason: 'Dental evaluation' }
          ],
          prescriptions: [
        ...
        },
        {
          ...
          labs: [
            { name: 'SDMA + Creatinine', code: 'KID001', status: 'completed', result: 'SDMA 12 µg/dL (ref 0-14), CREA 1.4 (ref 0.5-1.5), BUN 22 (ref 7-27). BUN improved from 35, creatinine stable at 1.4, SDMA within acceptable range. CKD stable with current management.', priority: 'Routine', testCategory: 'Laboratory', sampleType: 'Blood', billingStatus: 'paid', reason: 'CKD recheck - monitor kidney values' }
          ],
    ...
    },
    {
      ...
      visits: [
        ...
        },
        {
          ...
          labs: [
            { name: 'Allergy Test Panel', code: 'ALG001', status: 'completed', result: 'Positive reactions: Dust mites (+++), Grass pollen (++), Mold spores (++), Flea saliva (+). Negative: Food allergens (chicken, beef, dairy, wheat). Recommended: environmental management and immunotherapy.', priority: 'Routine', testCategory: 'Laboratory', sampleType: 'Serum', billingStatus: 'paid', reason: 'Chronic pruritus, rule out allergies' },
            { name: 'Skin Cytology', code: 'SKN001', status: 'completed', result: 'Ear swab: Yeast organisms (Malassezia) +++, Cocci bacteria ++. Skin scrape: No mites. Impression: Secondary pyoderma and Malassezia otitis secondary to allergic dermatitis.', priority: 'Routine', testCategory: 'Laboratory', sampleType: 'Skin swab', billingStatus: 'paid', reason: 'Pruritus with ear infection' }
          ],
          prescriptions: [
        ...
        },
        {
          ...
          labs: [
            { name: 'Skin Scraping', code: 'SKN002', status: 'completed', result: 'Deep and superficial skin scrapings negative for Demodex and Sarcoptes mites. Minimal cocci bacteria on cytology. Culture: No significant growth.', priority: 'Routine', testCategory: 'Laboratory', sampleType: 'Skin scraping', billingStatus: 'paid', reason: 'Recheck - monitor response to treatment' }
          ],
      ...
      visits: [
        {
          ...
          labs: [
            { name: 'Urinalysis', code: 'URIN002', status: 'completed', result: 'Color: Straw, Turbidity: Clear, pH 6.5, Specific Gravity 1.035, Protein Negative, Glucose Negative, WBC 0-2/hpf, RBC 3-5/hpf. No crystals or bacteria seen. Occasional RBC present consistent with Feline Idiopathic Cystitis.', priority: 'Urgent', testCategory: 'Laboratory', sampleType: 'Urine', billingStatus: 'not-billed', reason: 'Hematuria, dysuria, suspected FIC' },
            { name: 'Bladder Ultrasound', code: 'US001', status: 'completed', result: 'Bladder wall mildly thickened (4mm). No uroliths, masses, or sediment identified. Urethra normal. Impression: consistent with Feline Idiopathic Cystitis.', priority: 'Urgent', testCategory: 'Imaging', sampleType: 'Ultrasound', billingStatus: 'not-billed', reason: 'Confirm FIC diagnosis, rule out stones' }
          ],
          prescriptions: [
        ...
        },
        {
          ...
          labs: [
            { name: 'FeLV/FIV Snap Test', code: 'FEL001', status: 'completed', result: 'FeLV p27 antigen: Negative. FIV antibody: Negative. Cat is negative for both Feline Leukemia and Feline Immunodeficiency Virus.', priority: 'Routine', testCategory: 'Laboratory', sampleType: 'Blood', billingStatus: 'paid', reason: 'New kitten screening' }
          ],
          diagnosis: 'Healthy - Annual Wellness'
        }
      ]
      ...
      visits: [
        {
          ...
          labs: [
            { name: 'Thoracic X-Ray', code: 'XRY002', status: 'completed', result: 'Lungs: Mild peribronchial pattern. No alveolar infiltrates, no pleural effusion. Cardiac silhouette within normal limits. Impression: Mild bronchial disease, no pneumonia.', priority: 'Routine', testCategory: 'Imaging', sampleType: 'Radiograph', billingStatus: 'paid', reason: 'Chronic cough workup' },
            { name: 'Tracheal Wash Culture', code: 'TWC001', status: 'completed', result: 'Culture: Bordetella bronchiseptica isolated. Heavy growth. Sensitivity: Doxycycline S, Amoxicillin R, Enrofloxacin S.', priority: 'Routine', testCategory: 'Laboratory', sampleType: 'Tracheal wash', billingStatus: 'paid', reason: 'Identify cause of chronic cough' }
          ],
          prescriptions: [
        ...
        },
        {
          ...
          labs: [
            { name: 'Thyroid Function Test', code: 'THR001', status: 'completed', result: 'TT4: 2.8 µg/dL (ref 1.0-4.0). Free T4: 1.2 ng/dL (ref 0.6-2.0). TSH: 0.15 ng/mL (ref 0.0-0.6). Interpretation: Thyroid function within normal limits — hypothyroidism ruled out.', priority: 'Routine', testCategory: 'Laboratory', sampleType: 'Blood', billingStatus: 'paid', reason: 'Weight gain despite caloric restriction, rule out thyroid disease' },
            { name: 'Complete Blood Count', code: 'CBC004', status: 'completed', result: 'WBC 8.2 (ref 6-17), RBC 5.8 (ref 5.5-8.5), HGB 13.5 (ref 12-18). All values within normal reference ranges.', priority: 'Routine', testCategory: 'Laboratory', sampleType: 'Blood', billingStatus: 'paid', reason: 'Wellness screening' }
          ],
          prescriptions: [
      ...
      visits: [
        {
          ...
          labs: [
            { name: 'Pre-Surgical Panel', code: 'PSP001', status: 'completed', result: 'PCV 42% (ref 37-55), TP 6.5 (ref 6.0-7.5), BUN 18 (ref 7-27), CREA 0.9 (ref 0.5-1.5), ALT 52 (ref 10-100), ALP 85 (ref 20-150). All values within normal range — cleared for surgery.', priority: 'Routine', testCategory: 'Laboratory', sampleType: 'Blood', billingStatus: 'paid', reason: 'Pre-surgical screening' },
            { name: 'Stifle X-Ray', code: 'XRY003', status: 'completed', result: 'Lateral and AP views of left stifle: Complete cranial cruciate ligament rupture with mild joint effusion. No meniscal mineralization. Tibial plateau angle 26°. Recommend TPLO surgery.', priority: 'Routine', testCategory: 'Imaging', sampleType: 'Radiograph', billingStatus: 'paid', reason: 'Lameness left hind leg' }
          ],
          prescriptions: [
        ...
        },
        {
          ...
          labs: [
            { name: 'Post-Op X-Ray', code: 'XRY004', status: 'completed', result: 'TPLO plate and 4 screws in good position. Tibial plateau angle corrected from 26° to 9°. No implant complications. Mild soft tissue swelling expected post-op.', priority: 'Routine', testCategory: 'Imaging', sampleType: 'Radiograph', billingStatus: 'paid', reason: 'Post-op implant position verification' }
          ],
          prescriptions: [
            { name: 'Carprofen 75mg', dosage: '1 tablet', frequency: 'Twice daily', duration: '14 days', qty: 28, price: 45, dispensed: true },
        ...
        },
        {
          ...
          labs: [
            { name: 'Post-Surgery X-Ray', code: 'XRY005', status: 'completed', result: 'Implant in good position, no complications. Tibial plateau angle stable at 9°. Progressive bone healing. No implant loosening or migration.', priority: 'Routine', testCategory: 'Imaging', sampleType: 'Radiograph', billingStatus: 'paid', reason: '4-week post-op recheck' }
          ],
          prescriptions: [
  ...
  // Get Drive access token for sample lab report uploads
  const driveToken = await getDriveAccessToken();
  if (driveToken) {
    console.log('   [GDrive] Token obtained, will upload sample lab reports\n');
  } else {
    console.log('   [GDrive] No token — skipping lab report file uploads\n');
  }
    ...
    for (const visit of scenario.visits) {
      ...
      for (const svc of visit.services) {
        ...
        const serviceRef = await addDoc(collection(db, 'appointment_services'), {
          ...
          serviceCatalogId: '',
          serviceCode: (svc.type.substring(0, 4) + '-001').toUpperCase(),
      ...
      if (visit.labs) {
        for (const lab of visit.labs) {
          const owner = users[pet.ownerUid];
          const labRef = await addDoc(collection(db, 'lab_orders'), {
            encounterId,
            patientId: pet.id,
            petName: pet.name,
            ownerId: pet.ownerUid || '',
            ...
            testCategory: lab.testCategory || 'Laboratory',
            sampleType: lab.sampleType || 'Blood',
            billingStatus: lab.billingStatus || 'not-billed',
            reason: lab.reason || '',
            resultFileUrls: lab.resultFileUrls || []
          });

          // Upload sample lab report to Drive if token is available and lab is completed
          if (driveToken && lab.status === 'completed' && lab.result) {
            const ownerNameStr = users[pet.ownerId]?.displayName || users[pet.ownerUid]?.displayName || pet.ownerId || 'Owner';
            const fileUrl = await uploadSampleLabReport(
              driveToken, 
              {
                id: labRef.id,
                orderNo: `LAB-${labRef.id.slice(-6).toUpperCase()}`,
                petName: pet.name,
                ownerName: ownerNameStr,
                testName: lab.name,
                testCategory: lab.testCategory || 'Laboratory',
                resultSummary: lab.result,
                orderedAt: { toDate: () => visitDate },
                completedAt: { toDate: () => visitDate },
                orderedBy: doctors.find(d => d.id === visit.doctorId)?.name || visit.doctorId,
                completedBy: 'EdvirontVet Lab',
                reason: lab.reason,
                sampleType: lab.sampleType
              },
              pet,
              users[pet.ownerId] || users[pet.ownerUid] || { displayName: ownerNameStr }
            );
            if (fileUrl) {
      ...
      if (visit.prescriptions) {
        for (const rx of visit.prescriptions) {
          // Find matching med
          const medMatch = createdMeds.find(m => m.name.includes(rx.name) || rx.name.includes(m.name));
          
          const rxRef = await addDoc(collection(db, 'prescriptions'), {
            encounterId,
            patientId: pet.id,
            medicationName: rx.name,
            medicationCatalogId: medMatch ? medMatch.id : '',
            dosage: rx.dosage,
          ...
          if (rx.dispensed) {
            ...
            if (medMatch) {
              const medId = medMatch.id;
              // Find first batch
              const batchMatch = batches.find(b => b.medicationId === medId && b.quantity >= rx.qty);
              if (batchMatch) {
                const newQty = batchMatch.quantity - rx.qty;
                batchMatch.quantity = newQty;

                await updateDoc(doc(db, 'inventory_batches', batchMatch.id), {
                  quantity: newQty,
                  updatedAt: serverTimestamp()
                });
                ...
                await addDoc(collection(db, 'stock_movements'), {
                  medicationId: medId,
                  medicationName: medMatch.name,
                  batchId: batchMatch.id,
                  batchNo: batchMatch.batchNo,
                  type: 'dispensing',
                  quantity: -rx.qty,
                  runningBalance: newQty,
                  reference: `RX-${rxRef.id.substring(0,6)}`,
                  notes: `Dispensed for Encounter ${encounterId.substring(0,6)}`,
                  userId: 'system',
                  userName: 'System Admin',
                  createdAt: visitDate,
                });
                ...
                // Update total stock
                const totalStock = batches.filter(b => b.medicationId === medId).reduce((sum, b) => sum + b.quantity, 0);
