...
async function uploadSampleLabReport(token: string, ownerName: string, petName: string, testName: string, resultSummary: string): Promise<string> {
  try {
    ...
    const safeName = `${petName}_${testName.replace(/[^a-zA-Z0-9]/g, '_')}.txt`.slice(0, 100);
    const fileContent = `EdvirontVet Veterinary Clinic — Lab Report
========================================
Patient: ${petName}
Owner: ${ownerName}
Test: ${testName}
...
--- End of Report ---
Generated by EdvirontVet CRM Seed Script
`;
    ...
    const metadataResp = await fetch('https://www.googleapis.com/drive/v3/files?fields=id,name,webViewLink', {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ name: safeName, mimeType: 'text/plain', parents: [parentId] }),
    });
      ...
      if (visit.labs) {
        for (const lab of visit.labs) {
          ...
          if (driveToken && lab.status === 'completed' && lab.result) {
            const ownerName = users[pet.ownerId]?.displayName || users[pet.ownerUid]?.displayName || pet.ownerId || 'Owner';
            const fileUrl = await uploadSampleLabReport(driveToken, ownerName, pet.name, lab.name, lab.result);
            if (fileUrl) {
