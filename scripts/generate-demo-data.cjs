...
async function getTargetUserUid() {
...
function generatePetData(index, ownerUid) {
...
function generateAppointment(pet, index, services) {
  const now = new Date();
  ...
  const statusChoices = ['confirmed', 'unconfirmed', 'completed', 'cancelled'];
    ...
    for (let a = 0; a < APPT_PER_PET; a++) {
      ...
      if (appointment.status === 'completed') {
        ...
        const invoice = {
          ...
          date: appointment.date,
          dueDate: invoiceStatus === 'active' ? formatISO(addDays(new Date(appointment.date), DUE_OFFSET_DAYS), { representation: 'date' }) : appointment.date,
